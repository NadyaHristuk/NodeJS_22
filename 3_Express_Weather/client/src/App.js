...
class App extends Component {
	...
	getInfoMapClick = (e) => {
		...
		const URL = 'localhost:3002/weather/latlon';
		console.log(URL);
		axios
			.get(URL)
			.then((res) => {
	...
	searchImages() {
		...
		const URL = `http://localhost:3002/weather/images`;
		console.log(URL);
		axios
	...
	updateWeather() {
		...
		const URL = 'http://localhost:3002/weather/city';
		axios
			.get(`http://localhost:3002/weather/city?city=${cityName}`)
			.then((res) => {
				return res.data;
			})
			.then((data) => {
	...
	updateWeatherAndImage(cityName) {
		// const { cityName } = this.state;
		// let UpWeather = axios.get(`http://localhost:3002/weather/city?city=${cityName}`);
		// let UpImg = axios.post(`http://localhost:3002/weather/images`, {
		// 	city: cityName
		// });
		// Promise.all([ UpWeather, UpImg ])

		const URL = 'http://localhost:3002/weather/imageweather';
		axios.post(URL, { city: cityName }).then((res) => {
			console.log(res);
			this.setState({
				isLoading: false,
